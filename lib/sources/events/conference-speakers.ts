// Conference Speakers — Conference speaker lists
...
export class ConferenceSpeakersSourceSource extends BaseSource {
  ...
  async search(query: string, options?: SearchOptions): Promise<Lead[]> {
    ...
    for (let i = 0; i < count; i++) {
      leads.push(this.makeLead({
        ...
        tags: ["events", "conference-speakers"],
        metadata: { source: "Conference Speakers", description: "Conference speaker lists" },
      }))
    }
    return leads
  }
  ...
  async getCompany(domain: string): Promise<CompanyData | null> {
    return this.makeCompany({
      ...
      description: "Company data from Conference Speakers",
      industry: "Technology",
    })
  }
