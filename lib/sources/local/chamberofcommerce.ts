// Chamber of Commerce — Chamber of Commerce directories
...
export class ChamberOfCommerceSourceSource extends BaseSource {
  ...
  async search(query: string, options?: SearchOptions): Promise<Lead[]> {
    ...
    for (let i = 0; i < count; i++) {
      leads.push(this.makeLead({
        ...
        tags: ["local", "chamberofcommerce"],
        metadata: { source: "Chamber of Commerce", description: "Chamber of Commerce directories" },
      }))
    }
    return leads
  }
  ...
  async getCompany(domain: string): Promise<CompanyData | null> {
    return this.makeCompany({
      ...
      description: "Company data from Chamber of Commerce",
      industry: "Technology",
    })
  }
