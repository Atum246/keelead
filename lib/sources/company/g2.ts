// G2 — G2.com software reviews/companies
import { BaseSource } from "../base"
import type { Lead, SearchOptions, CompanyData, ContactData } from "../types"

export class G2SourceSource extends BaseSource {
  name = "G2"
  id = "g2"
  category = "company"
  requiresApiKey = true
  ...
  async search(query: string, options?: SearchOptions): Promise<Lead[]> {
    ...
    for (let i = 0; i < count; i++) {
      leads.push(this.makeLead({
        ...
        tags: ["company", "g2"],
        metadata: { source: "G2", description: "G2.com software reviews/companies" },
      }))
    }
    return leads
  }
  ...
  async getCompany(domain: string): Promise<CompanyData | null> {
    return this.makeCompany({
      ...
      description: "Company data from G2",
      industry: "Technology",
    })
  }
...
export default new G2SourceSource()
