// USPTO Trademarks — USPTO Trademarks
...
export class TrademarksSourceSource extends BaseSource {
  ...
  async search(query: string, options?: SearchOptions): Promise<Lead[]> {
    ...
    for (let i = 0; i < count; i++) {
      leads.push(this.makeLead({
        ...
        tags: ["government", "trademarks"],
        metadata: { source: "USPTO Trademarks", description: "USPTO Trademarks" },
      }))
    }
    return leads
  }
  ...
  async getCompany(domain: string): Promise<CompanyData | null> {
    return this.makeCompany({
      ...
      description: "Company data from USPTO Trademarks",
      industry: "Technology",
    })
  }
