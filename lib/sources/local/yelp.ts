...
export class YelpSource2Source extends BaseSource {
  name = "Yelp"
  id = "yelp"
  category = "local"
  requiresApiKey = true
  ...
  async search(query: string, options?: SearchOptions): Promise<Lead[]> {
    ...
    for (let i = 0; i < count; i++) {
      leads.push(this.makeLead({
        ...
        tags: ["local", "yelp"],
        metadata: { source: "Yelp", description: "Yelp Fusion API" },
      }))
    }
    return leads
  }
  ...
  async getCompany(domain: string): Promise<CompanyData | null> {
    return this.makeCompany({
      ...
      description: "Company data from Yelp",
      industry: "Technology",
    })
  }
...
export default new YelpSource2Source()
