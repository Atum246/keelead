// Indeed — Job postings reveal company info
import { BaseSource } from "../base"
import type { Lead, SearchOptions, CompanyData, ContactData } from "../types"

export class IndeedSourceSource extends BaseSource {
  name = "Indeed"
  id = "indeed"
  category = "company"
  requiresApiKey = true
  ...
  async search(query: string, options?: SearchOptions): Promise<Lead[]> {
    ...
    for (let i = 0; i < count; i++) {
      leads.push(this.makeLead({
        ...
        tags: ["company", "indeed"],
        metadata: { source: "Indeed", description: "Job postings reveal company info" },
      }))
    }
    return leads
  }
  ...
  async getCompany(domain: string): Promise<CompanyData | null> {
    return this.makeCompany({
      ...
      description: "Company data from Indeed",
      industry: "Technology",
    })
  }
...
export default new IndeedSourceSource()
