// AngelList — AngelList/Wellfound startup data
import { BaseSource } from "../base"
import type { Lead, SearchOptions, CompanyData, ContactData } from "../types"

export class AngelListSource2Source extends BaseSource {
  name = "AngelList"
  id = "angellist"
  ...
  async search(query: string, options?: SearchOptions): Promise<Lead[]> {
    ...
    for (let i = 0; i < count; i++) {
      leads.push(this.makeLead({
        ...
        tags: ["professional", "angellist"],
        metadata: { source: "AngelList", description: "AngelList/Wellfound startup data" },
      }))
    }
    return leads
  }
  ...
  async getCompany(domain: string): Promise<CompanyData | null> {
    return this.makeCompany({
      ...
      description: "Company data from AngelList",
      industry: "Technology",
    })
  }
...
export default new AngelListSource2Source()
