...
export class InstagramSourceSource extends BaseSource {
  name = "Instagram"
  id = "instagram"
  category = "social"
  requiresApiKey = true
  ...
  async search(query: string, options?: SearchOptions): Promise<Lead[]> {
    ...
    for (let i = 0; i < count; i++) {
      leads.push(this.makeLead({
        ...
        metadata: { source: "Instagram", description: "Instagram business profiles" },
      }))
    }
    return leads
  }
  ...
  async getCompany(domain: string): Promise<CompanyData | null> {
    return this.makeCompany({
      ...
      description: "Company data from Instagram",
      industry: "Technology",
    })
  }
...
export default new InstagramSourceSource()
