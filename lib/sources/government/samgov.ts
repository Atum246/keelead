// SAM.gov — SAM.gov US government contractors
import { BaseSource } from "../base"
import type { Lead, SearchOptions, CompanyData, ContactData } from "../types"

export class SAMGovSourceSource extends BaseSource {
  name = "SAM.gov"
  id = "samgov"
  category = "government"
  ...
  async search(query: string, options?: SearchOptions): Promise<Lead[]> {
    ...
    for (let i = 0; i < count; i++) {
      leads.push(this.makeLead({
        ...
        tags: ["government", "samgov"],
        metadata: { source: "SAM.gov", description: "SAM.gov US government contractors" },
      }))
    }
    return leads
  }
  ...
  async getCompany(domain: string): Promise<CompanyData | null> {
    return this.makeCompany({
      ...
      description: "Company data from SAM.gov",
      industry: "Technology",
    })
  }
...
export default new SAMGovSourceSource()
