...
export class EURegisterSourceSource extends BaseSource {
  ...
  async search(query: string, options?: SearchOptions): Promise<Lead[]> {
    ...
    for (let i = 0; i < count; i++) {
      leads.push(this.makeLead({
        ...
        tags: ["government", "eu-register"],
        metadata: { source: "EU Transparency Register", description: "EU Transparency Register" },
      }))
    }
    return leads
  }
  ...
  async getCompany(domain: string): Promise<CompanyData | null> {
    return this.makeCompany({
      ...
      description: "Company data from EU Transparency Register",
      industry: "Technology",
    })
  }
