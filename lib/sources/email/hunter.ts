...
export class HunterSource2Source extends BaseSource {
  name = "Hunter.io"
  id = "hunter"
  category = "email"
  requiresApiKey = true
  ...
  async search(query: string, options?: SearchOptions): Promise<Lead[]> {
    ...
    for (let i = 0; i < count; i++) {
      leads.push(this.makeLead({
        ...
        tags: ["email", "hunter"],
        metadata: { source: "Hunter.io", description: "Hunter.io email finder" },
      }))
    }
    return leads
  }
  ...
  async getCompany(domain: string): Promise<CompanyData | null> {
    return this.makeCompany({
      ...
      description: "Company data from Hunter.io",
      industry: "Technology",
    })
  }
...
export default new HunterSource2Source()
