// Stack Overflow — Stack Overflow developers
...
export class StackOverflowSourceSource extends BaseSource {
  ...
  async search(query: string, options?: SearchOptions): Promise<Lead[]> {
    ...
    for (let i = 0; i < count; i++) {
      leads.push(this.makeLead({
        ...
        tags: ["developer", "stackoverflow"],
        metadata: { source: "Stack Overflow", description: "Stack Overflow developers" },
      }))
    }
    return leads
  }
  ...
  async getCompany(domain: string): Promise<CompanyData | null> {
    return this.makeCompany({
      ...
      description: "Company data from Stack Overflow",
      industry: "Technology",
    })
  }
