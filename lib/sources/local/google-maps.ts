...
export class GoogleMapsSource2Source extends BaseSource {
  name = "Google Maps"
  id = "google-maps"
  category = "local"
  requiresApiKey = true
  ...
  async search(query: string, options?: SearchOptions): Promise<Lead[]> {
    ...
    for (let i = 0; i < count; i++) {
      leads.push(this.makeLead({
        ...
        tags: ["local", "google-maps"],
        metadata: { source: "Google Maps", description: "Google Places API" },
      }))
    }
    return leads
  }
  ...
  async getCompany(domain: string): Promise<CompanyData | null> {
    return this.makeCompany({
      ...
      description: "Company data from Google Maps",
      industry: "Technology",
    })
  }
...
export default new GoogleMapsSource2Source()
