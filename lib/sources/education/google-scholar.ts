// Google Scholar — Academic researchers
...
export class GoogleScholarSourceSource extends BaseSource {
  ...
  async search(query: string, options?: SearchOptions): Promise<Lead[]> {
    ...
    for (let i = 0; i < count; i++) {
      leads.push(this.makeLead({
        ...
        tags: ["education", "google-scholar"],
        metadata: { source: "Google Scholar", description: "Academic researchers" },
      }))
    }
    return leads
  }
  ...
  async getCompany(domain: string): Promise<CompanyData | null> {
    return this.makeCompany({
      ...
      description: "Company data from Google Scholar",
      industry: "Technology",
    })
  }
