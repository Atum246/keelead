// Eventbrite — Eventbrite attendees/speakers
import { BaseSource } from "../base"
import type { Lead, SearchOptions, CompanyData, ContactData } from "../types"

export class EventbriteSourceSource extends BaseSource {
  name = "Eventbrite"
  id = "eventbrite"
  category = "events"
  requiresApiKey = true
  ...
  async search(query: string, options?: SearchOptions): Promise<Lead[]> {
    ...
    for (let i = 0; i < count; i++) {
      leads.push(this.makeLead({
        ...
        metadata: { source: "Eventbrite", description: "Eventbrite attendees/speakers" },
      }))
    }
    return leads
  }
  ...
  async getCompany(domain: string): Promise<CompanyData | null> {
    return this.makeCompany({
      ...
      description: "Company data from Eventbrite",
      industry: "Technology",
    })
  }
...
export default new EventbriteSourceSource()
