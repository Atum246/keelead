...
export class HomeAdvisorSourceSource extends BaseSource {
  name = "HomeAdvisor"
  id = "homeadvisor"
  category = "local"
  requiresApiKey = true
  ...
  async search(query: string, options?: SearchOptions): Promise<Lead[]> {
    ...
    for (let i = 0; i < count; i++) {
      leads.push(this.makeLead({
        ...
        tags: ["local", "homeadvisor"],
        metadata: { source: "HomeAdvisor", description: "Home services directory" },
      }))
    }
    return leads
  }
  ...
  async getCompany(domain: string): Promise<CompanyData | null> {
    return this.makeCompany({
      ...
      description: "Company data from HomeAdvisor",
      industry: "Technology",
    })
  }
...
export default new HomeAdvisorSourceSource()
