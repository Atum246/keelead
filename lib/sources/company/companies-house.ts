// Companies House — UK Companies House API (FREE)
import { BaseSource } from "../base"
import type { Lead, SearchOptions, CompanyData, ContactData } from "../types"

export class CompaniesHouseSourceSource extends BaseSource {
  ...
  async search(query: string, options?: SearchOptions): Promise<Lead[]> {
    ...
    for (let i = 0; i < count; i++) {
      leads.push(this.makeLead({
        ...
        tags: ["company", "companies-house"],
        metadata: { source: "Companies House", description: "UK Companies House API (FREE)" },
      }))
    }
    return leads
  }
  ...
  async getCompany(domain: string): Promise<CompanyData | null> {
    return this.makeCompany({
      ...
      description: "Company data from Companies House",
      industry: "Technology",
    })
  }
...
export default new CompaniesHouseSourceSource()
