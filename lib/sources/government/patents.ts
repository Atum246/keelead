// USPTO Patents — USPTO/Google Patents
...
export class PatentsSourceSource extends BaseSource {
  ...
  async search(query: string, options?: SearchOptions): Promise<Lead[]> {
    ...
    for (let i = 0; i < count; i++) {
      leads.push(this.makeLead({
        ...
        tags: ["government", "patents"],
        metadata: { source: "USPTO Patents", description: "USPTO/Google Patents" },
      }))
    }
    return leads
  }
  ...
  async getCompany(domain: string): Promise<CompanyData | null> {
    return this.makeCompany({
      ...
      description: "Company data from USPTO Patents",
      industry: "Technology",
    })
  }
