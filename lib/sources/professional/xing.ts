// Xing — Xing professional network
import { BaseSource } from "../base"
import type { Lead, SearchOptions, CompanyData, ContactData } from "../types"

export class XingSourceSource extends BaseSource {
  name = "Xing"
  id = "xing"
  ...
  async search(query: string, options?: SearchOptions): Promise<Lead[]> {
    ...
    for (let i = 0; i < count; i++) {
      leads.push(this.makeLead({
        ...
        tags: ["professional", "xing"],
        metadata: { source: "Xing", description: "Xing professional network" },
      }))
    }
    return leads
  }
  ...
  async getCompany(domain: string): Promise<CompanyData | null> {
    return this.makeCompany({
      ...
      description: "Company data from Xing",
      industry: "Technology",
    })
  }
...
export default new XingSourceSource()
