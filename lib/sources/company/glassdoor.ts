...
export class GlassdoorSourceSource extends BaseSource {
  name = "Glassdoor"
  id = "glassdoor"
  category = "company"
  requiresApiKey = true
  ...
  async search(query: string, options?: SearchOptions): Promise<Lead[]> {
    ...
    for (let i = 0; i < count; i++) {
      leads.push(this.makeLead({
        ...
        tags: ["company", "glassdoor"],
        metadata: { source: "Glassdoor", description: "Company reviews and salaries" },
      }))
    }
    return leads
  }
  ...
  async getCompany(domain: string): Promise<CompanyData | null> {
    return this.makeCompany({
      ...
      description: "Company data from Glassdoor",
      industry: "Technology",
    })
  }
...
export default new GlassdoorSourceSource()
