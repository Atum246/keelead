...
export class ThumbtackSourceSource extends BaseSource {
  name = "Thumbtack"
  id = "thumbtack"
  category = "local"
  requiresApiKey = true
  ...
  async search(query: string, options?: SearchOptions): Promise<Lead[]> {
    ...
    for (let i = 0; i < count; i++) {
      leads.push(this.makeLead({
        ...
        tags: ["local", "thumbtack"],
        metadata: { source: "Thumbtack", description: "Local service providers" },
      }))
    }
    return leads
  }
  ...
  async getCompany(domain: string): Promise<CompanyData | null> {
    return this.makeCompany({
      ...
      description: "Company data from Thumbtack",
      industry: "Technology",
    })
  }
...
export default new ThumbtackSourceSource()
