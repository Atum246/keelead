// Product Hunt — Product Hunt makers/products
import { BaseSource } from "../base"
import type { Lead, SearchOptions, CompanyData, ContactData } from "../types"

export class ProductHuntSource2Source extends BaseSource {
  name = "Product Hunt"
  id = "producthunt"
  category = "startup"
  requiresApiKey = true
  ...
  async search(query: string, options?: SearchOptions): Promise<Lead[]> {
    ...
    for (let i = 0; i < count; i++) {
      leads.push(this.makeLead({
        ...
        metadata: { source: "Product Hunt", description: "Product Hunt makers/products" },
      }))
    }
    return leads
  }
  ...
  async getCompany(domain: string): Promise<CompanyData | null> {
    return this.makeCompany({
      ...
      description: "Company data from Product Hunt",
      industry: "Technology",
    })
  }
...
export default new ProductHuntSource2Source()
