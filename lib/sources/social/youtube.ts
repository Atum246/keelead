...
export class YouTubeSourceSource extends BaseSource {
  name = "YouTube"
  id = "youtube"
  category = "social"
  requiresApiKey = true
  ...
  async search(query: string, options?: SearchOptions): Promise<Lead[]> {
    ...
    for (let i = 0; i < count; i++) {
      leads.push(this.makeLead({
        ...
        tags: ["social", "youtube"],
        metadata: { source: "YouTube", description: "YouTube channel data" },
      }))
    }
    return leads
  }
  ...
  async getCompany(domain: string): Promise<CompanyData | null> {
    return this.makeCompany({
      ...
      description: "Company data from YouTube",
      industry: "Technology",
    })
  }
...
export default new YouTubeSourceSource()
