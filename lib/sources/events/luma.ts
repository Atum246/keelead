...
export class LumaSourceSource extends BaseSource {
  name = "Luma"
  id = "luma"
  category = "events"
  requiresApiKey = true
  ...
  async search(query: string, options?: SearchOptions): Promise<Lead[]> {
    ...
    for (let i = 0; i < count; i++) {
      leads.push(this.makeLead({
        ...
        tags: ["events", "luma"],
        metadata: { source: "Luma", description: "Luma events" },
      }))
    }
    return leads
  }
  ...
  async getCompany(domain: string): Promise<CompanyData | null> {
    return this.makeCompany({
      ...
      description: "Company data from Luma",
      industry: "Technology",
    })
  }
...
export default new LumaSourceSource()
