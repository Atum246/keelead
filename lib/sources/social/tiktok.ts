// TikTok — TikTok creator profiles
import { BaseSource } from "../base"
import type { Lead, SearchOptions, CompanyData, ContactData } from "../types"

export class TikTokSourceSource extends BaseSource {
  name = "TikTok"
  id = "tiktok"
  category = "social"
  requiresApiKey = true
  ...
  async search(query: string, options?: SearchOptions): Promise<Lead[]> {
    ...
    for (let i = 0; i < count; i++) {
      leads.push(this.makeLead({
        ...
        tags: ["social", "tiktok"],
        metadata: { source: "TikTok", description: "TikTok creator profiles" },
      }))
    }
    return leads
  }
  ...
  async getCompany(domain: string): Promise<CompanyData | null> {
    return this.makeCompany({
      ...
      description: "Company data from TikTok",
      industry: "Technology",
    })
  }
...
export default new TikTokSourceSource()
