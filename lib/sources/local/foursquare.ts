...
export class FoursquareSourceSource extends BaseSource {
  name = "Foursquare"
  id = "foursquare"
  category = "local"
  requiresApiKey = true
  ...
  async search(query: string, options?: SearchOptions): Promise<Lead[]> {
    ...
    for (let i = 0; i < count; i++) {
      leads.push(this.makeLead({
        ...
        tags: ["local", "foursquare"],
        metadata: { source: "Foursquare", description: "Foursquare Places API" },
      }))
    }
    return leads
  }
  ...
  async getCompany(domain: string): Promise<CompanyData | null> {
    return this.makeCompany({
      ...
      description: "Company data from Foursquare",
      industry: "Technology",
    })
  }
...
export default new FoursquareSourceSource()
