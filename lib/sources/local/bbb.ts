// Better Business Bureau — BBB business directory
...
export class BBBSourceSource extends BaseSource {
  ...
  async search(query: string, options?: SearchOptions): Promise<Lead[]> {
    ...
    for (let i = 0; i < count; i++) {
      leads.push(this.makeLead({
        ...
        tags: ["local", "bbb"],
        metadata: { source: "Better Business Bureau", description: "BBB business directory" },
      }))
    }
    return leads
  }
  ...
  async getCompany(domain: string): Promise<CompanyData | null> {
    return this.makeCompany({
      ...
      description: "Company data from Better Business Bureau",
      industry: "Technology",
    })
  }
